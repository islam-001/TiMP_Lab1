...
modShifr::modShifr(int skey, string text) {
    key = getValidKey(skey, text);
}

string modShifr :: encryption(string &text) {
...
inline string modShifr::getValidCipherText(string &s, string &open_text) {
    int r1 = s.size();
...
string modShifr :: transcript(string &text, string &open_text) {
...
inline string modShifr::getValidOpenText(string &s) {
    int razmer = s.size();
...
inline int modShifr::getValidKey(int key,const string &Text) {
